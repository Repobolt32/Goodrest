...
- **Global scope**: `E:/desktop/goodrest/.gemini/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `E:/desktop/goodrest/.gemini/notes/`)
...
2. Glob `E:/desktop/goodrest/.gemini/notes/*.md` (if directory exists) — global notes
...
Global (E:/desktop/goodrest/.gemini/notes/):
...
2. **No `.planning/`**: Falls back to global `E:/desktop/goodrest/.gemini/notes/` — works in any directory
