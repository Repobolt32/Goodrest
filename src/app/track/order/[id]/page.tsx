...
export default function SingleOrderPage() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`order-sync-${params.id}-${uniqueId}`)
      .on(
        'postgres_changes',
        {
          event: 'UPDATE',
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col pb-12">
      ...
      <main className="flex-1 p-6 max-w-4xl mx-auto w-full grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <section className="space-y-6">
          ...
          <OrderTracker
            orderId={order.id}
            initialStatus={order.order_status || 'placed'}
            initialRiderPhone={order.rider_phone}
            durationSeconds={order.duration_seconds}
            riderStartedAt={order.rider_started_at}
            orderLat={order.lat}
            orderLng={order.lng}
            initialCancelledBy={order.cancelled_by}
