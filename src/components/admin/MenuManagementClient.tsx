...
export default function MenuManagementClient({
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (res.success && res.data) {
        setRadius(res.data.max_delivery_radius ?? 10);
      }
    };
    init();
  }, []);
  ...
  const handleUpdateRadius = async () => {
    ...
    const result = await updateAppSettings({ max_delivery_radius: radius });
    if (!result.success) {
      alert('Failed to update radius: ' + result.error);
    } else {
      alert('Delivery radius updated successfully!');
    }
      ...
      <div className="flex flex-col gap-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white p-5 md:p-8 rounded-3xl border border-slate-100 shadow-sm relative overflow-hidden group">
          <div className="relative z-10">
            <h1 className="text-2xl font-black text-slate-900 mb-1 uppercase tracking-tight">Menu Management</h1>
        ...
        <div className="bg-white p-6 md:p-8 rounded-3xl border border-slate-100 shadow-sm">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-6">
            <div className="flex-1 w-full">
              <label htmlFor="maxRadius" className="block text-xs font-bold text-slate-500 mb-2">Maximum Delivery Radius (Kilometers)</label>
              <div className="relative">
                <input
                  ...
                  className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl p-4 pl-12 font-black text-slate-900 focus:border-primary outline-none transition-all"
                  placeholder="e.g. 10"
                />
                <span className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 font-bold">KM</span>
              </div>
            </div>
            <button
              onClick={handleUpdateRadius}
              disabled={radiusLoading}
              className="w-full sm:w-auto bg-slate-900 text-white px-8 py-4 rounded-2xl font-black uppercase tracking-widest hover:bg-slate-800 transition-all flex items-center justify-center gap-2"
            >
              {radiusLoading ? <Loader2 className="animate-spin" size={20} /> : <Save size={20} />}
              Save Radius
            </button>
          </div>
          <p className="mt-3 text-[10px] font-bold text-slate-400 uppercase tracking-tight">
            Orders beyond this distance will be automatically rejected at checkout.
          </p>
        </div>
      </div>
              ...
              {items
                ...
                .map((item) => (
                  <motion.div
                    ...
                    className={`bg-white rounded-[2rem] md:rounded-[2.5rem] p-4 md:p-6 border border-slate-100 shadow-sm hover:shadow-xl hover:shadow-slate-200/50 transition-all group relative overflow-hidden ${!item.is_available ? 'grayscale opacity-75' : ''
                      }`}
                  >
                    <div className="flex items-start gap-6 relative z-10">
                      ...
                      <div className="flex-1">
                        <div className="flex items-start justify-between mb-2">
                          ...
                          <div className="flex items-center gap-1">
                            ...
                            <button
                              ...
                              className={`p-2 rounded-xl transition-all ${item.is_available
                        ...
                        <div className="flex items-center gap-2 mb-4">
                          <span className={`text-[10px] font-black uppercase tracking-widest px-2 py-0.5 rounded-md ${item.is_available ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-500'
                            }`}>
